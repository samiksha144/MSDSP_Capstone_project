...
# ---- Unified add_audit wrapper (DB-first with v2→v1 fallback) ----
def add_audit(actor: str, event: str, detail: str = "") -> None:
    """
    Writes to SQL via dbo.sp_add_audit using db_repo.add_audit.
    Tries new signature: (@actor,@actor_role,@admin_id,@user_id,@event,@detail)
    Falls back to old signature: (@actor,@user_id,@event,@detail)
    If DB fails, appends to in-memory buffer so UI still shows something.
    """
...
def _get_openai_key() -> Optional[str]:
    # ⚠️ replace with your own key management
    return ""
...
# ---------------------------
# Sidebar (API status + whoami)
# ---------------------------
with st.sidebar:
    st.subheader("OpenAI status")
    key_present = bool(_get_openai_key())
    st.write("API key:", "✅ found" if key_present else "❌ missing")
    st.caption("For production, store the key in an environment variable or .streamlit/secrets.toml")
    st.markdown("---")
...
with left:
    ...
    with c1:
        st.download_button("Download PDF", data=st.session_state.summary_pdf or b"", file_name="sop_summary.pdf",
                           ...
                           on_click=lambda: add_audit("User", "Download", "sop_summary.pdf"))
    with c2:
